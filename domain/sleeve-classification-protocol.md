...
*   **套管與牆/樓板相交（第一層碰撞排除）**：
    *   主模型或連結模型中的 Walls 或 Floors 實體與套管重疊。
    *   處理：**直接排除**，不回傳亦不在圖面上標註。
*   **套管長度與梁寬不符（第二層幾何篩選）**：
    *   套管與梁相交，但套管長度與梁寬誤差 $> 10\text{mm}$（例如 $14314610$）。
    *   處理：標記為 `EXCLUDED_WARNING`，將其從 Revit 平面圖的圖面標籤標註中**剔除**，但必須在**明細表與文字報告中保留並顯示 `⚠️ EXCLUDED_WARNING`**，並註明原因（如「套管長度與梁寬不匹配，疑似穿牆套管」）以提醒使用者。
